...
def compute_loss(predictions, reid_predictions, targets, model, n_obj):  # predictions, targets, model
    # j: calculate three losses and a total - lbox, lobj, lcls and loss (total)
    device = targets.device
    _, lbox, lobj, lreid = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    _, tbox, indices, anchors, reid_classes = build_targets(predictions, targets, model)  # targets
    ...
    for layer_index, layer_predictions in enumerate(predictions):  # j: for every of the 3 YOLO output layers
        ...
        if num_targets:
            ...
            if reid_predictions != None:  # j: False in validation mode (we do not use main loss to validate reids)
                ...
                reid_label = torch.zeros([num_targets, n_obj]).to("cuda")
                ...
                preid = reid_predictions[b, :, grid_j_mult, grid_i_mult]  # j:... take 4,2,1 consecutive target series
                preid = preid.log()
                reid_layer_loss = (preid * reid_label).sum() / n_obj / -num_targets
                lreid += reid_layer_loss / 3 # j: 1/3 because we sum over 3 scales
            else:
    ...
    lobj *= (3. / 2)
    # lcls *= 0.31
    lcls = torch.zeros(1, requires_grad=False).to("cuda")
    batch_size = tobj.shape[0]  # batch size

    # loss = (lbox + lobj + lcls) + (0.5 * lreid)  # j: weighing ReID loss like in the YOLO-ReID paper
    loss = (lbox + lobj + 0) + (0.5 * lreid)  # j: weighing ReID loss like in the YOLO-ReID paper

    return loss * batch_size, to_cpu(torch.cat((lbox, lobj, lcls, loss, lreid)))
