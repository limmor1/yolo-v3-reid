...
def detect_image(model, image, use_cuda,
    ...
    # Configure input
    # j: the transformation methods work on training data which includes bounding boxes so we pass dummy bounding box
    input_img, _ = transforms.Compose([
        DEFAULT_TRANSFORMS, 
        Resize(img_size)])(
            # (image, np.zeros((1, 5))))[0].unsqueeze(0)
            (image, np.zeros((1, 6))))  # j: move indexes to accommodate unique ID
    ...
    with torch.no_grad():
        # j: YOLO-REID
        detections, reid_outputs = model(input_img)  # N x 85, where rows are (x1, y1, x2, y2, conf, cls1, cls2, ..., clsN)
        # j: YOLO-80
        # detections = model(input_img)  # N x 85, where rows are (x1, y1, x2, y2, conf, cls1, cls2, ..., clsN)
        # TODO: debug 0class uncomment
        # cls_ids = detections[:, 5]
        # mask = cls_ids == 0
        # detections = detections[mask]
        detections = non_max_suppression(detections, conf_thres, nms_thres)  # N-suppressed x 6, where rows are (x1, y1, x2, y2, conf, cls)
        detections = rescale_boxes(detections[0], img_size, image.shape[:2])

    # j: YOLO-REID
    return to_cpu(detections).numpy(), [to_cpu(reid_output).numpy() for reid_output in reid_outputs]
    # j: YOLO-80
